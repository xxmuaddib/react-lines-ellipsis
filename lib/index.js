...
var _require2 = require('./helpers'),
    omit = _require2.omit,
    string_as_unicode_escape = _require2.string_as_unicode_escape;
  ...
  _createClass(LinesEllipsis, [{
    ...
    value: function reflow(props) {
      ...
      this.canvas.innerHTML = this.units.map(function (c) {
        var char = string_as_unicode_escape(c);
        return "<span class='LinesEllipsis-unit'>".concat(char, "</span>");
      }).join('');
    ...
    value: function calcIndexes() {
      ...
      while (elt = elt.nextElementSibling) {
        ...
        for (var a = 65; a <= 65 + 26 * 2; a++) {
          alphabet.push(a);
        }

        elt.innerHTML = alphabet.includes(parseInt(elt.innerHTML.replace(/\\u/g, '', 16))) ? String.fromCharCode(parseInt(elt.innerHTML.replace(/\\u/g, '', 16))) : 'i';
